...
def add_sample_data(dataframe: pd.DataFrame, sample_id: str) -> pd.DataFrame:
    ...
    dataframe = (
        dataframe
        .add_column("id", sample_id).reorder_columns(["id"])
        .reorder_columns(["id", "chrom", "start", "end", "num.mark",
                          "seg.mean.adj", "call"])
    )
...
def convert_to_seg(segments: pd.DataFrame, bins: pd.DataFrame,
                   aberrations: pd.DataFrame,
                   sample: Optional[str] = None) -> pd.DataFrame:
    ...
    result = (
        segments_bed.intersect(bins_bed, F=1, c=True)
        .to_dataframe(disable_auto_names=True,
                      names=["chrom", "start", "end", "seg.mean",
                             "zscore", "num.mark"])
    )
    ...
    if aberrations.empty:
        ...
        result = (
            result
            .reorder_columns(["chrom", "start", "end",
                              "num.mark", "seg.mean", "zscore"])
    ...
    result = (
        BedTool
        .from_dataframe(result.remove_columns("zscore"))
        .intersect(aberrations_bed, loj=True)
    )
    ...
    result = (
       ...
       .to_dataframe(disable_auto_names=True,
                     names=["chrom", "start", "end", "seg.mean",
                            "num.mark", "a_chr", "a_start", "a_end",
                            "a_logr", "a_zscore", "call"]
                     )
       .process_text("call", string_function="replace", pat=".",
                     repl="neut", regex=False)
       .select_columns(["chrom", "start", "end", "num.mark", "seg.mean",
                        "call"])
       #  Zero logR if the call is neutral
       .join_apply(lambda x: 0 if x.call == "neut" else x["seg.mean"],
...
def main() -> None:
    ...
    parser.add_argument("--id", metavar="SAMPLEID",
                        help="Add SAMPLEID to the converted file")
    ...
    converted_df = convert_to_seg(segments, bins, aberrations,
                                  sample=options.id)
